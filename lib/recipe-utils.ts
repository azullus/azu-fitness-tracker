...
export function suggestServingsForPerson(
  ...
  const baseServings = recipe.baseServings ?? recipe.servings ?? 1;
  const caloriesPerServing = recipe.macrosPerServing?.calories ??
    recipe.calories ?? (recipe.nutrition?.calories ?? 400) / baseServings;
...
export function generateShoppingList(
  ...
  for (const recipe of recipes) {
    const scaleFactor = householdSize / (recipe.baseServings ?? recipe.servings ?? 2);
