...
// Helper to get item name (supports both 'name' and 'item' field names from different schemas)
function getItemDisplayName(pantryItem?: PantryItem): string {
  return pantryItem?.name || pantryItem?.item || '';
}

export function PantryItemForm({ item, onSubmit, onCancel }: PantryItemFormProps) {
  const isEditMode = !!item;

  const [name, setName] = useState<string>(getItemDisplayName(item));
  ...
  useEffect(() => {
    if (item) {
      setName(getItemDisplayName(item));
